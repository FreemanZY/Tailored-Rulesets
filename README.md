...
`china_enterprise_manual_domainset.txt` keeps manually observed Alibaba-ecosystem endpoints that are not covered by the reviewed core suffixes. `wechat_manual_domainset.txt` keeps manually observed functional WeChat/Weixin endpoints. `force_direct_domainset.txt` and `reject_domainset.txt` contain other reviewed one-off direct and rejection observations. Exact hosts remain exact unless the source explicitly used suffix semantics; generated actions never modify these manual files.

The legacy import deliberately excludes private infrastructure names, conflicting direct copies of telemetry endpoints, broad duplicate MMTLS regular expressions, and four historical CDN `/32` observations whose present ownership and exclusivity were not established. The maintained `force_direct_ruleset.txt` retains the narrower validated MMTLS URL rule.
